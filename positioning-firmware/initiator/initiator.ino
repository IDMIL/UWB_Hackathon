...
const char * pass = "mappings";
const char * addr = "192.168.1.255";
const int    port = 1234;
...
constexpr size_t id_idx = 9;
static uint8_t ids[] = {0,1,2,3};
...
void loop() {
    ...
    static OSCMessage &rx0 = bndl.add("/rx0");
    static OSCMessage &rx1 = bndl.add("/rx1");
    static OSCMessage &rx2 = bndl.add("/rx2");
    static OSCMessage &rx3 = bndl.add("/rx3");
    static OSCMessage * rx[] = {&rx0, &rx1, &rx2, &rx3};
        /* Write frame data to DW IC and prepare transmission. See NOTE 7 below. */
        static uint8_t tx_id = 0;
        static uint8_t tries = 0;
        printf("tx_id %d\n", tx_id);
        tx_poll_msg[ALL_MSG_SN_IDX] = frame_seq_nb;
        tx_poll_msg[id_idx] = tx_id;
        ...
        if (status_reg & SYS_STATUS_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                uint8_t id = rx_buffer[id_idx];
                printf("rx id %d\n", id);
                rx_resp_msg[id_idx] = id;
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    rx[id]->set(0, (float)distance);

                    /* Display computed distance on LCD. */
                    snprintf(dist_str, sizeof(dist_str), "DIST %d: %3.2f m", id, distance);
        ...
        else
        {
            ...
            tries++;
            if (tries > 5) {if (++tx_id >= 4) tx_id = 0; tries = 0;}
