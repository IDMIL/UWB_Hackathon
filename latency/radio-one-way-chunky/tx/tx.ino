...
void setup() {
  for (int i = 0; i < sizeof(tx_msg)-1; ++i) tx_msg[i] = i;
  UART_init();
...
void loop() {
    ...
    while (true)
    {
        ...
        statushi = dwt_read32bitreg(SYS_STATUS_HI_ID);
        if ((status & SYS_STATUS_TXFRS_BIT_MASK)) break;
        if ((status & SYS_STATUS_HPDWARN_BIT_MASK) || (statushi & SYS_STATUS_HI_CMD_ERR_BIT_MASK)) break;
        delay(1);
    };
    ...
    else
    {
        ...
        if (status & SYS_STATUS_HPDWARN_BIT_MASK)
            dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_HPDWARN_BIT_MASK);
        if (statushi & SYS_STATUS_HI_CMD_ERR_BIT_MASK)
            dwt_write32bitreg(SYS_STATUS_HI_ID, SYS_STATUS_HI_CMD_ERR_BIT_MASK);
